...
def has_image_provider() -> bool:
    """Check if any image provider API key is configured."""
    return bool(
...
class TestImageGenerationRequest:
    ...
    def test_create_with_model_and_prompt(self):
        ...
        req = ImageGenerationRequest("openai/dall-e-3", "A serene landscape")
        assert req is not None
        assert repr(req) is not None

    def test_with_n(self):
        """Test setting number of images."""
        req = ImageGenerationRequest("openai/dall-e-3", "A cat")
        req_with_n = req.with_n(3)
        assert req_with_n is not None

    def test_with_size(self):
        """Test setting image size."""
        req = ImageGenerationRequest("openai/dall-e-3", "A dog")
    ...
    def test_with_negative_prompt(self):
        ...
        req = ImageGenerationRequest("stability-ai/stable-diffusion-xl", "A sunset")
    ...
    def test_with_seed(self):
        ...
        req = ImageGenerationRequest("fal-ai/flux/dev", "A mountain")
    ...
    def test_builder_chain(self):
        ...
        req = (
            ImageGenerationRequest("openai/dall-e-3", "A magical forest")
            .with_n(2)
    ...
    def test_various_prompts(self):
        ...
        for prompt in prompts:
            req = ImageGenerationRequest("openai/dall-e-3", prompt)
            assert req is not None
...
class TestImageClientMethods:
    ...
    @pytest.mark.skipif(not has_image_provider(), reason="No image provider API key configured")
    def test_generate_image_with_request(self):
        """Test calling generate_image with ImageGenerationRequest."""
        client = LLMKitClient.from_env()
        req = ImageGenerationRequest("openai/dall-e-3", "A beautiful sunset over mountains")
    ...
    def test_generate_image_response_properties(self):
        ...
        client = LLMKitClient.from_env()
        req = ImageGenerationRequest("openai/dall-e-3", "Test image generation")
        response = client.generate_image(req)  # type: ignore[attr-defined]
    ...
    def test_generate_image_with_configured_options(self):
        ...
        client = LLMKitClient.from_env()
        req = (
            ImageGenerationRequest("openai/dall-e-3", "An animated scene")
            .with_n(2)
    ...
    def test_generate_image_error_handling(self):
        ...
        client = LLMKitClient.from_env()
        req = ImageGenerationRequest("openai/dall-e-3", "")  # Empty prompt
...
class TestImageIntegration:
    ...
    def test_end_to_end_image_generation_flow(self):
        ...
        # 1. Create request
        req = ImageGenerationRequest("openai/dall-e-3", "A serene Japanese garden with temple")
    ...
    def test_multiple_image_generations(self):
        ...
        prompts = [
            ("openai/dall-e-3", "A cat playing with yarn"),
    ...
    def test_image_size_variations(self):
        ...
        for size in sizes:
            req = ImageGenerationRequest("openai/dall-e-3", "Test image").with_size(size)
            response = client.generate_image(req)  # type: ignore[attr-defined]
            assert isinstance(response, ImageGenerationResponse)
