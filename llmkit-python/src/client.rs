...
impl PyLLMKitClient {
    ...
    ///
    ///     # Custom retry
    ///     client = LLMKitClient.from_env(retry_config=RetryConfig.conservative())
    ///
    ///     # Disable retry
        ...
        match result {
            Ok(status) => {
                let (video_url, status_str) = match &status {
                    llmkit::VideoJobStatus::Completed { video_url, .. } => {
                    ...
                    llmkit::VideoJobStatus::Queued => (None, Some("queued".to_string())),
                    llmkit::VideoJobStatus::Failed { error, .. } => {
                        (None, Some(format!("failed: {}", error)))
                    }
                    llmkit::VideoJobStatus::Cancelled => (None, Some("cancelled".to_string())),
                };
    ...
    fn generate_image_with_provider(
        ...
        if let Some(size) = request.size {
            let image_size = match size {
                crate::image::PyImageSize::Square256 => llmkit::ImageSize::Square256,
                crate::image::PyImageSize::Square512 => llmkit::ImageSize::Square512,
                crate::image::PyImageSize::Square1024 => llmkit::ImageSize::Square1024,
                crate::image::PyImageSize::Portrait1024x1792 => {
                    llmkit::ImageSize::Portrait1024x1792
                }
                crate::image::PyImageSize::Landscape1792x1024 => {
                    llmkit::ImageSize::Landscape1792x1024
                }
            };
        ...
        if let Some(quality) = request.quality {
            let image_quality = match quality {
                crate::image::PyImageQuality::Hd => llmkit::ImageQuality::Hd,
                crate::image::PyImageQuality::Standard => llmkit::ImageQuality::Standard,
            };
