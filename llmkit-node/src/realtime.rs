...
impl FromNapiValue for JsVadConfig {
    unsafe fn from_napi_value(
        ...
        let silence_duration_ms: u32 = obj.get::<u32>("silenceDurationMs")?.unwrap_or(500);
        let threshold: Option<f64> = obj.get::<f64>("threshold")?;
        Ok(Self {
            silence_duration_ms,
            threshold,
        })
...
impl Default for JsSessionConfig {
    fn default() -> Self {
        Self {
            model: None,
...
impl JsRealtimeProvider {
    ...
    pub async fn create_session(&self, config: JsSessionConfig) -> napi::Result<JsRealtimeSession> {
